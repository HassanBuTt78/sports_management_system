 ...
 * coach/team/view.php
 * ------------------------------------------------------------
 * Team details for a coach — but ONLY their own team. The
 * ownership check (t.coach_id = session coach_id) happens in
 * the SQL itself, not as an afterthought: if the WHERE clause
 * doesn't match, $team is simply null and the page redirects,
 * so there's no code path where a coach can view someone else's
 * team by editing the URL.
 * ============================================================
 */
...
if ($conn && $teamId > 0) {
    $stmt = $conn->prepare(
        'SELECT t.*, s.sport_name, cap.full_name AS captain_name, vc.full_name AS vice_captain_name
         FROM teams t
    ...
    if ($team) {
        $stmt = $conn->prepare(
            'SELECT p.player_id, p.full_name, p.profile_image, p.position, p.status, pa.average_rating
             ...
             WHERE p.team_id = ? ORDER BY pa.average_rating DESC, p.full_name'
        );
...
<div class="container py-5">
  <div class="page-heading">
    ...
    <div class="d-flex gap-2">
      <a href="<?php echo BASE_URL; ?>/coach/team/edit.php?id=<?php echo $teamId; ?>" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-pen me-1"></i>Edit &amp; Manage Roster</a>
      <a href="<?php echo BASE_URL; ?>/coach/team/index.php" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-arrow-left me-1"></i>Back</a>
    </div>
  </div>
  ...
  <div class="grid-2col">
    <div class="panel text-center">
      ...
      <table class="table table-borderless text-start mt-3">
        <tbody>
          <tr><th style="width:150px;">Captain</th><td><?php echo safeOut($team['captain_name'] ?? '—'); ?></td></tr>
  ...
  <div class="panel">
    ...
    <?php if (empty($members)): ?>
      <p class="text-muted mb-0">No players on this team yet. <a href="<?php echo BASE_URL; ?>/coach/team/edit.php?id=<?php echo $teamId; ?>">Add some &rarr;</a></p>
    <?php else: ?>
          ...
          <tbody>
            <?php foreach ($members as $m): ?>
              <tr>
                ...
                <td><?php echo safeOut($m['full_name']); ?></td>
