 ...
 * Expects $conn and an active coach session (requireRole('coach')
...
if ($conn) {
    $stmt = $conn->prepare(
        "SELECT notification_id, title, message, status, created_at
         FROM notifications
         WHERE receiver_role IN ('coach','all') AND (receiver_id = ? OR receiver_id IS NULL)
    ...
    $stmt = $conn->prepare(
        "SELECT COUNT(*) c FROM notifications
         WHERE receiver_role IN ('coach','all') AND (receiver_id = ? OR receiver_id IS NULL) AND status = 'unread'"
    );
    $stmt->bind_param('i', $coachId);
    $stmt->execute();
  ...
  <div class="topbar-actions">
    ...
    <div class="dropdown">
      ...
      <div class="dropdown-menu dropdown-menu-end topbar-dropdown profile-dropdown">
        <a href="<?php echo BASE_URL; ?>/coach/profile.php" class="dropdown-item-custom"><i class="fa-solid fa-user"></i> My Profile</a>
...
<script>
/* Notification read/delete for the coach topbar bell — posts to the
   role-agnostic notifications/mark_read.php endpoint (ownership is
   verified server-side against the logged-in coach's session). */
(function () {
